...
headings.forEach((text) => {
  slugger.reset(); // Slugger keeps history for duplicates, reset for independent check?
  // Actually rehype-slug uses one slugger per document, so duplicates matter.
  // But here we are checking single instances.
  // Actually, let's keep it same instance to mimic one doc, but headings are unique here.
